...
export default function Reports() {
  const { medicines, dispenses } = useData();
  const [fromDate, setFromDate] = useState('2026-05-01');
  const [toDate, setToDate] = useState('2026-05-31');

  // Aggregate dispense data for preview
  const filteredDispenses = dispenses.filter(d => {
    const date = new Date(d.date);
    return date >= new Date(fromDate) && date <= new Date(toDate + 'T23:59:59');
  });

  const aggregated = filteredDispenses.reduce<Record<string, { medicineName: string; category: string; totalQty: number; times: number; unitPrice: number }>>((acc, d) => {
    if (!acc[d.medicineId]) {
      const med = medicines.find(m => m.id === d.medicineId);
      acc[d.medicineId] = { medicineName: d.medicineName, category: med?.category ?? '', totalQty: 0, times: 0, unitPrice: med?.unitPrice ?? 0 };
    }
    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-5 mb-8">
        {reportCards.map(({ icon: Icon, title, description, color }) => (
          <div key={title} className="bg-white dark:bg-[#1A1730] rounded-xl p-6 shadow-sm border border-transparent dark:border-[#2D2B45] card-hover"
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 mt-4">
                  <button className="flex items-center gap-1.5 px-3.5 py-2 text-xs font-medium text-white rounded-lg transition-colors btn-press"
                    style={{ background: color }}>
                    <FileText size={13} /> Generate
                  </button>
                  <button className="flex items-center gap-1.5 px-3.5 py-2 text-xs font-medium border rounded-lg transition-colors btn-press"
      ...
      <div className="bg-white dark:bg-[#1A1730] rounded-xl shadow-sm border border-transparent dark:border-[#2D2B45] overflow-hidden"
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-50 dark:border-[#2D2B45]">
          <div>
            <h3 className="text-base font-semibold text-gray-900 dark:text-white">Monthly Dispense Report — May 2026</h3>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Aggregated dispense data for selected date range</p>
          </div>
          <div className="flex items-center gap-3">
            ...
            <button className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-gray-600 dark:text-gray-400 border border-gray-200 dark:border-[#2D2B45] rounded-lg hover:bg-gray-50 dark:hover:bg-[#252240] transition-colors">
              <Printer size={13} /> Print
            </button>
            <button className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-white bg-[#7C3AED] hover:bg-[#5B21B6] rounded-lg transition-colors">
              <Download size={13} /> Export CSV
            </button>
          </div>
        </div>
            ...
            <tbody>
              {reportRows.length === 0 ? (
                ...
                <tr key={i}
                  ...
                  <td className="px-4 py-3 font-semibold text-[#7C3AED]">Rs.{(row.totalQty * row.unitPrice).toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
            {reportRows.length > 0 && (
              <tfoot>
                <tr className="border-t-2 border-gray-200 dark:border-[#2D2B45]" style={{ background: '#F5F3FF' }}>
                  <td colSpan={2} className="px-4 py-3 text-xs font-bold text-gray-700 dark:text-gray-300 uppercase">Total</td>
                  <td className="px-4 py-3 font-bold text-gray-900 dark:text-white">{reportRows.reduce((s, r) => s + r.totalQty, 0)}</td>
                  <td className="px-4 py-3 font-bold text-gray-900 dark:text-white">{reportRows.reduce((s, r) => s + r.times, 0)}</td>
                  <td />
                  <td className="px-4 py-3 font-bold text-[#7C3AED]">Rs.{reportRows.reduce((s, r) => s + r.totalQty * r.unitPrice, 0).toLocaleString()}</td>
                </tr>
              </tfoot>
            )}
          </table>
